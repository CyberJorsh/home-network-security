...
For addresses belonging to the computer running the app, discovery also reads the OS-reported hostname, MAC, hardware model, vendor, and operating-system version. These values are never assigned to neighboring devices. Failed OS inventory reads leave the corresponding fields unknown. macOS uses fixed system queries; Windows uses read-only CIM inventory.
